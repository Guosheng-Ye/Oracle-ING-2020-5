...
-   >**分类**
    ...
    -   **根据表空间存储方式可分为**
        ...
        -   本地管理方式(LOCAL)
            -   在本地管理方式中，区的分配和管理信息都存储在表空间的数据文件中，而与数据字典无关。表空间在每个数据文件中维护一个“位图”结构，用于记录表空间中所有区的分配情况，因此在分配与回收时，oracle将对数据文件中的位图进行更新，不会产生回滚信息或重做信息。

####    6.2 管理表空间(用户表空间)
#####   6.2.1 表空间的规划
-   >需要建表空间吗？数据文件放哪里？用户需要什么？怎么做到高效、高速和安全
-   >主要考虑的因素：设置操作系统目录结构、将碎片减到最小、将磁盘竞争减到最小、将段分开、数据库物理文件的分散存储
-   >**表空间的设计策略**
    ...
    -   能够将表空间设置为脱机状态或联机状态，以便对数据库的一部分内容进行备份或恢复
    -   能够将表空间设置为只读状态，从而将数据库的一部分设置为只读状态
    -   能够为某种特殊用途专门设置一个表空间，如临时表空间，优化表空间的使用效率
    -   能够更加灵活地为用户设置表空间配额
#####   6.2.2 创建表空间
-   >在创建本地管理方式下的表空间时，应该确定表空间的名称、类型、对应的数据文件的名称和位置以及分区的分配方式、段的管理方式
    -   表空间的名称不能超过30个字符，必须以字母开头，可以包含字母、数字以及一些特殊字符
    -   表空间的类型包括永久性表空间、临时表空间和撤销表空间
    -   表空间中区的分配方式包括两种：自动扩展(AUTOALLOCATE)和定制(UNIFORM)
    -   段的管理包括两种：自动管理和手动管理
#####   6.2.3 表空间的状态管理
#####   6.2.4 重命名表空间
#####   6.2.5 默认表空间
...
## 第8章 模式对象-索引
####    8.1 索引的感念与分类
-   >目录：通过有序的"小数据"的词汇表的快速查找，发现词条在"大数据量"的书籍中出现的位置
-   >索引：建立在存储表基础上，有助于快速定位所需记录的一种辅助存储结构
-   >**索引的原理** 
    -   由索引项组成，每个索引项包括**索引字段**和**行指针**
        -   索引字段：由Table的某些列(通常为一列)的值排列而成
        -   行指针：指向Table中包含索引字段值的记录在此案上的存储位置
    -   索引的关键在于通过一组排序后的索引字段来取代默认的全表扫描方式，从而提高效率
    -   Pointer：ROWID伪列
        -   Data object number  +  Relative file number + Block number + Row number
        -   
|index|pointer|     |EMPNO|ENAME|JOB |SAL|
|----|----    |-----|----|----  |----  |----|
|7088|        |  |->|7566|Justin|Manager|8500|
|7566|        | -|  |7088|John|Salesman|3500|
|7934|        | -|  |7960|Cherry|Salesman|4200|
|7960|        |  |->|7934|Margin|Check|4056|
| 索引|文件  |     |---|---主|文件--|---|
-   >**索引的特点**
    -   索引在逻辑上和物理上都与相关的表的数据*无关*，当创建或删除一个索引时，不会影响基本的表、数据库应用或其他索引
    -   索引一旦建立后，当在表上进行DML操作时，Oracle会自动维护索引，并决定何时使用索引。
    -   索引的使用度用户是透明的，用户并不需要在执行SQL语句时指定用哪个索引及如何使用索引，也就是说，无论表上是否创建有索引，SQL语句的用法不变，用户在进行操作时，不需要考虑索引的存在，索引只与系统性能有关
-   >**索引的分类**
    -   **索引文件组织形式**
        -   排序索引文件
        -   散列索引文件
    -   **索引数据的组织形式**
        -   B-Tree索引
            -   关系数据库中使用最多的索引
            -   最大的特点是无论表中的行存储在什么位置，都能够以相同的方法和速度进行查询
            -   分类
                -   Unique
                    -   唯一索引，其索引值不能重复，但允许为NULL，在创建索引时指定UNIQUE关键字可以创建唯一索引。当建立“主键约束条件”时Oracle会自动在相应的列上建立唯一索引
                -   Non-Unique
                    -   非唯一索引，其索引值可以重复，允许为NULL，默认情况下，Oracle创建的索引是非唯一索引
                -   Reverse Key
